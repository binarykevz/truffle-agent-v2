...
// ============================================================
// THREE SEPARATE TURSO DATABASES
// ============================================================

const commandsUrl = process.env.TURSO_COMMANDS_URL;
if (!commandsUrl) throw new Error("TURSO_COMMANDS_URL is required");
export const commandsDb: Client = createClient({
    url: commandsUrl,
    authToken: process.env.TURSO_COMMANDS_TOKEN || undefined,
});

const usersUrl = process.env.TURSO_USERS_URL;
if (!usersUrl) throw new Error("TURSO_USERS_URL is required");
export const usersDb: Client = createClient({
    url: usersUrl,
    authToken: process.env.TURSO_USERS_TOKEN || undefined,
});

const memoryUrl = process.env.TURSO_MEMORY_URL;
if (!memoryUrl) throw new Error("TURSO_MEMORY_URL is required");
export const memoryDb: Client = createClient({
    url: memoryUrl,
    authToken: process.env.TURSO_MEMORY_TOKEN || undefined,
});

// ============================================================
// SCHEMA INIT — ⭐ ALL THREE EXPORTED
// ============================================================
...
export async function getConfig(key: string): Promise<string | null> {
    const row = await memoryDb.execute({ sql: "SELECT value FROM config WHERE key = ?", args: [key] });
...
export async function deleteConfig(key: string): Promise<void> {
    await memoryDb.execute({ sql: "DELETE FROM config WHERE key = ?", args: [key] });
}

export async function getAllConfig(): Promise<Record<string, string>> {
    const rows = await memoryDb.execute("SELECT key, value FROM config");
...
export async function getOwner(): Promise<number | null> {
    const row = await usersDb.execute({ sql: "SELECT user_id FROM owner WHERE id = 1" });
...
export async function removeAllowedUser(userId: number): Promise<boolean> {
    const res = await usersDb.execute({ sql: "DELETE FROM allowed_users WHERE user_id = ?", args: [userId] });
    return (res.rowsAffected ?? 0) > 0;
}

export async function isAllowedUser(userId: number): Promise<boolean> {
    if (await isOwner(userId)) return true;
    const row = await usersDb.execute({ sql: "SELECT 1 FROM allowed_users WHERE user_id = ?", args: [userId] });
    return row.rows.length > 0;
}

export async function listAllowedUsers(): Promise<{ user_id: number; username: string | null; added_at: number }[]> {
    const rows = await usersDb.execute("SELECT user_id, username, added_at FROM allowed_users ORDER BY added_at ASC");
...
export async function clearHistory(userId: number): Promise<void> {
    await memoryDb.execute({ sql: "DELETE FROM history WHERE user_id = ?", args: [userId] });
}
